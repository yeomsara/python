...
MN = 'MONTHLY_BATCH_CODE_NOBRAND.py'
LG = 'EVENT NOBRAND'
...
#■=====================================================================
try : 
    parser = argparse.ArgumentParser(description = 'NOBRAND EVENT SIMILARITY BATCH')
...
try:
    ...
    result_tb = 'CDS_AMT.TB_AMT_EVENT_SIMILARITY_NOBRAND_RESULT'
    log_df = add_logs(module = '행사유사도(노브랜드)',mdl_type='운영',step='0.예측시작',qt='Create',tt=result_tb,
    ...
    #21.03.24 노브랜드 상품검색쿼리 수정(현업공유)
    NOBRAND_PRODUCT='''SELECT DISTINCT PRDT_NM AS MASTER_PRDT_NM,PRDT_CD
                        FROM
                        (
    ...
    FROM CDS_DW.TB_DW_RCIPT_DETAIL AS R
        ...
        LEFT JOIN 
                                    ...
                                    FROM CDS_DW.TB_DW_RCIPT_DETAIL AS R
                                        ...
                                        AND R.AFLCO_CD=D.AFLCO_CD
                                        AND P.PRDT_DCODE_CD = D.PRDT_DCODE_CD
                                        LEFT JOIN ({NOBRAND_PRODUCT}) AS N
    ...
    NOBRAND=query_SQL( query1 )    
    
    del_sql = f'''DELETE FROM {result_tb} WHERE YM_WCNT='{YM_WCNT}' '''
    execute_query(del_sql)
    insert_table(result_tb,NOBRAND)
    end_date = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
    log_df = add_logs(module = '행사유사도(노브랜드)',mdl_type='운영',step='1.APPLY 테이블 생성 및 결과적재',qt='Insert',tt=result_tb,
    ...
    logger.error(error)
    end_date = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
    log_df = add_logs(module = '행사유사도(노브랜드)',mdl_type='운영',step='error',qt='error',tt='==== error occured ====',
