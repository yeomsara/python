...
TRUNCATE TABLE  TB_AMT_RETENTION_APPLY;

INSERT INTO TB_AMT_RETENTION_APPLY
(WITH  
RETENTION_TARGET1 AS (
SELECT :YM_BAIN AS BAIN_GRADE_YM, A2.CUST_ID
, A1.GRADE_CD AS BAIN_GRADE_CD 
...
--DROP TABLE CDS_AMT.TB_AMT_RETENTION_RESULT

CREATE COLUMN TABLE CDS_AMT.TB_AMT_RETENTION_RESULT		
(				
	BAIN_GRADE_YM	NVARCHAR(6)	NOT NULL  COMMENT	'베인등급 기준연월'
,	CUST_ID	NVARCHAR(20)	NOT NULL  COMMENT	'고객번호'
,	BAIN_GRADE_CD	NVARCHAR(2)	NOT NULL  COMMENT	'회원등급_베인등급 코드'
,	PREDICT_PROB	DECIMAL(30,6)	COMMENT	'이탈예측 확률'
,	PREDICT_PROB_GROUP	TINYINT	COMMENT	'이탈예측 확률 0.5이상 시 1 그 외 0'
,	DATA_CRTN_DT	DATE	COMMENT	'데이터생성일자'
, 	CONSTRAINT TB_AMT_RETENTION_RESULT PRIMARY KEY (BAIN_GRADE_YM,CUST_ID,BAIN_GRADE_CD)			
)
UNLOAD PRIORITY 5 AUTO MERGE COMMENT '(분석마트)Retention모델 월별 고객이탈예측결과' 
...
SELECT BAIN_GRADE_YM ,BAIN_GRADE_CD ,count(DISTINCT CUST_ID) AS cus, avg(PREDICT_PROB)
